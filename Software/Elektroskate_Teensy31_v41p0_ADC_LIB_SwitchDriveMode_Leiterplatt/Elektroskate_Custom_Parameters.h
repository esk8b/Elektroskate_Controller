...
#define YAchseUT               125 // Wert der Nunchuk Y-Achse unterhalb dessen die Beschleunigung zurueckgenommen wird(Integrationssteuerung) (0..65535, 115..124)
#define YAchseOT               131 // Wert der Nunchuk Y-Achse oberhalb dessen beschleunigt wird(Integrationssteuerung) (0..65535, 141..145)
...
#define XAchseMin (uint16_t)                3 // MinmalWert X-Achse ganz unten (0..65535,0..30)
#define XAchseMax (uint16_t)              240 // MinmalWert X-Achse ganz unten (0..65535,0..30)
...
// Temperatursensoren
#define defTempSensorAnzahl (uint8_t)       0 // Die Temperatursensoranzahl hier eintragen
uint8_t defTempAddress[2][8] = {{ 0x28, 0x75, 0x3F, 0x2A, 0x04, 0x00, 0x00, 0x73 }     // Seriennummer der Temperatursensoren, diese muss extra ermittelt werden
                               ,{ 0x28, 0x8C, 0x4C, 0x52, 0x04, 0x00, 0x00, 0x00 }};   // Seriennummer der Temperatursensoren, diese muss extra ermittelt werden
							   
// PWM Motorsteuerung
#define PWM_ObererStellWertMotorIgel 2.0  // Wert berechnen fuer obere Pulsweite Igel (ms Pulsweite) (0.0..2.5, 1.6..1.8)
...
#define BattSpgMMax (float) 56.1 // Maximale messbare Batterienspannung (Koeffizienten im Ausgleichspolynom muessen angepasst werden) 
                                 // 32.7V (10k Ohm / 2 x 180k Ohm) 
                                 // 40V   (10k Ohm / 121k Ohm) 
                                 // 42.9V (10k Ohm / 120k Ohm)
                                 // 52.8V (10k Ohm / 150k Ohm)
                                 // 56.1V (10k Ohm / 160k Ohm) -> Dr. Board
#define StromMMax   (float) 200.0 // Maximal messbarer Strom in A (ACS756SCA-050B Stromsensor)
...
// Koeffizienten fuer 42.9V Wertebereich (Gltigkeitsbereich 20-42 V)
 ...
 #define UBatPoly  (float) (Ubatt) // Dr. Board 56.1V kein Polynom vorhanden.

// Strombegrenzung und Kurzschlusserkennung
#define Ishort  (uint16_t) 100   // Maximaler Strom, dieser fuehrt zur sofortigen Abschaltung (<= StromMMax)
#define Imax    (uint16_t) 80    // Grenzwert, ab dem die Strombegrenzung einsetzt
#define Strom0A (float)    200  // 0A Messwert, wird angezeigt wenn kein Strom fliesst und Strom0A = 0 (muss kalibriert werden)

// Innenwiderstandsberechnung
#define Ischwellwert (uint16_t)  3.0  // Dieser Mindeststrom muss ueberschritten damit der Innenwiderstand berechnet wird 
#define Idelta       (uint16_t)  5.0  // Erst wenn Ischwellwert und Idelta zwischen zwei Messunngen erreicht wird, wird die Berechnung ausgefuehrt
