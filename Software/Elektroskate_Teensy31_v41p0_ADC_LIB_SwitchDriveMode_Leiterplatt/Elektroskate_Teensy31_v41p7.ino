...
void setup()
{
  ...
  //Notaus Eingang definieren
  pinMode(Pin_Notaus, INPUT_PULLUP);         // Notaus-Eingang aktivieren
...
void Motorsteuerung() {
  ...
  //Wurde der Notaus ausgelöst, Motor sofort auf Neutralstellung
  if (digitalRead(Pin_Notaus) == true) {
    StellWertMotor = LeerlaufStellWertMotor;  // wurde Notaus ausgelöst?
    if (DEBUG_Motorsteller) Serial_B.print("NOTAUS");
  }
